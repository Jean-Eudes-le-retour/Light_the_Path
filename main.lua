...
--grid.fit(t,xpos,ypos,state,rotation,color,canMove,canRotate,canChangeColor,glassState,canChangeState)
  ...
  for i=1,grid_dim_x do
    grid.set(nil,i,1,nil,nil,nil,nil,nil,nil,0)
  end
  for i=1,grid_dim_x do
    grid.set(TYPE_WALL,i,grid_dim_y)
  end
  for i=1,grid_dim_y do
    grid.set(TYPE_WALL,1,i,nil,nil,nil,nil,nil,nil,0)
  end
  for i=1,grid_dim_y do
    grid.set(TYPE_WALL,grid_dim_x,i,nil,nil,nil,nil,nil,nil,0)
  end
  ...
  grid.fit(TYPE_SOURCE,nil,nil,1,3,COLOR_BLACK,true,true)
  grid.fit(TYPE_SOURCE,nil,nil,2,3,(math.floor(math.random()*20)%7)+1,true,true)
  grid.fit(TYPE_SOURCE,nil,nil,2,3,(math.floor(math.random()*20)%7)+1,true,true)
  grid.fit(TYPE_RECEIVER,nil,nil,1,1,COLOR_BLACK,true,true)
  grid.fit(TYPE_RECEIVER,nil,nil,2,1,3)
  grid.fit(TYPE_MIRROR,nil,nil,1,1,2)
  grid.fit(TYPE_MIRROR,nil,nil,1,1,4)
  grid.fit(TYPE_MIRROR,nil,nil,2,1,7)
  grid.fit(TYPE_MIRROR,nil,nil,2,1,COLOR_WHITE)
  grid.fit(TYPE_MIRROR,nil,nil,2,1,COLOR_WHITE)
  grid.fit(TYPE_MIRROR,nil,nil,2,1,COLOR_WHITE)
  grid.fit(TYPE_MIRROR,nil,nil,2,1,COLOR_BLACK)
  grid.fit(TYPE_PWHEEL,nil,nil,1,1,5)
  grid.fit(TYPE_PWHEEL,nil,nil,2,1,7)
  grid.fit(TYPE_LOGIC,nil,nil,LOGIC_OR,nil,nil,true,true):setSides(nil,"in","in","out")
  grid.fit(TYPE_LOGIC,nil,nil,LOGIC_AND,nil,nil,true,true):setSides("in","in","in","out")
  grid.fit(TYPE_LOGIC,nil,nil,LOGIC_AND,nil,nil,true,true):setSides("in",nil,"in","out")
  grid.fit(TYPE_LOGIC,nil,nil,LOGIC_NOT,nil,nil,true,true):setSides(nil,"in","in","out")
  grid.fit(TYPE_RECEIVER,nil,nil,nil,nil,COLOR_BLACK,true,true):setSides(nil,nil,"activate",nil)
  grid.set(TYPE_RECEIVER,17,8,nil,3,COLOR_WHITE,false,false,false,true):setSides(nil,nil,"activate",nil)
  grid.set(TYPE_SOURCE,18,8,nil,2,COLOR_WHITE,false,false,false,true,false)
  grid.set(TYPE_MIRROR,16,9,2,1,COLOR_CYAN,false,false,false,true,false)
  grid.set(TYPE_DELAY,17,9,2,nil,nil,false,false,false,true,false)
  grid.set(TYPE_LOGIC,16,8,LOGIC_NOT,nil,COLOR_WHITE,false,false,false,true,false):setSides(nil,"out","in")
  grid.set(TYPE_LOGIC,18,9,LOGIC_AND,nil,COLOR_BLACK,false,false,false,true,false):setSides("in","out",nil,"in")
  grid.set(TYPE_RECEIVER,19,9,nil,3,COLOR_RED,false,false,false,true,false)
  grid.set(TYPE_GLASS,19,8)
  grid.fit(TYPE_DELAY,nil,nil,60)
  for i=1,50 do
    grid.fit(TYPE_DELAY,nil,nil,5)
  end
