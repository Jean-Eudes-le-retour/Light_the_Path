...
--grid.fit(t,xpos,ypos,state,rotation,color,canMove,canRotate,canChangeColor,glassState)
  for i=1,3 do
    grid.set(TYPE_WALL, 5, i)
  end
  grid.set(TYPE_WALL, 7, 3)
  grid.set(TYPE_WALL, 8, 3)
  grid.set(TYPE_WALL, 7, 1)

  grid.set(TYPE_SOURCE, 1, 4, {rotation = 1, color = COLOR_RED})
  grid.set(TYPE_SOURCE, 1, 5, {rotation = 1,color = COLOR_BLUE})
  grid.set(TYPE_SOURCE, 11, 4, {rotation = 3,color = COLOR_YELLOW})

  grid.set(TYPE_RECEIVER, 7, 4, {rotation = 1, color = COLOR_WHITE}):setSides(nil,nil,"activate",nil)  
  grid.set(TYPE_RECEIVER, 6, 1, {rotation = 2, color = COLOR_RED})
  grid.set(TYPE_RECEIVER, 4, 6, {color = COLOR_GREEN})
  
  grid.set(TYPE_MIRROR, 6, 4, {rotation = 1, state = 2, color = COLOR_YELLOW, glass=true})
  
  grid.set(TYPE_PWHEEL, 9, 5, {rotation = 1, state = 2, color = COLOR_CYAN})
  
  --player tiles
  grid.set(TYPE_MIRROR, 1, 1, {color = COLOR_GREEN})
  grid.set(TYPE_MIRROR, 1, 2, {color = COLOR_GREEN})
  grid.set(TYPE_MIRROR, 2, 1, {color = COLOR_WHITE})
  grid.set(TYPE_MIRROR, 2, 2, {color = COLOR_CYAN})
